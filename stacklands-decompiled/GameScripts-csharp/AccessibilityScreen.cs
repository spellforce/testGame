...
public class AccessibilityScreen : SokScreen
{
	...
	private void Update()
	{
		...
		((TMP_Text)AutoPauseWhenUsingControllerButton.TextMeshPro).text = SokLoc.Translate("label_auto_pause_controller") + " " + OptionsScreen.YesNo(AutoPauseWhenUsingController);
		((TMP_Text)AutoPauseWhenUsingKeyboardMouseButton.TextMeshPro).text = SokLoc.Translate("label_auto_pause_keyboardmouse") + " " + OptionsScreen.YesNo(AutoPauseWhenUsingKeyboardMouse);
		((TMP_Text)ScreenShakeButton.TextMeshPro).text = SokLoc.Translate("label_screenshake_enabled") + ": " + OptionsScreen.YesNo(ScreenshakeEnabled);
		((TMP_Text)ClickToDragButton.TextMeshPro).text = SokLoc.Translate("label_clicktodrag_enabled") + ": " + OptionsScreen.YesNo(ClickToDragEnabled);
		((TMP_Text)DisablePausedTextButton.TextMeshPro).text = SokLoc.Translate("label_disable_paused_text", (LocParam[])(object)new LocParam[1] { LocParam.Create("on_off", OptionsScreen.YesNo(FlashingPausedEnabled)) });
