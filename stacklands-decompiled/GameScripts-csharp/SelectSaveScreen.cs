...
public class SelectSaveScreen : SokScreen
{
	...
	private void TryCreateSaveButtons()
	{
		if (saveButtons.Count <= 0)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				((Component)customButton).transform.SetParentClean((Transform)(object)ButtonsParent);
	...
	private void UpdateButtonText()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			if (save != null)
			{
				((TMP_Text)customButton.TextMeshPro).text = SaveManager.instance.GetSaveSummary(save);
			}
			else
			{
				((TMP_Text)customButton.TextMeshPro).text = SokLoc.Translate("label_start_new_save", (LocParam[])(object)new LocParam[1] { LocParam.Create("save_index", (index + 1).ToString()) });
			}
