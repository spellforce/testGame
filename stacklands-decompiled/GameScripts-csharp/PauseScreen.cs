...
public class PauseScreen : SokScreen
{
	...
	private void Awake()
	{
		...
		((Component)NewCardopediaEntryRect).gameObject.SetActive(false);
	}

	private void Update()
	{
		((Component)NewCardopediaEntryRect).gameObject.SetActive(WorldManager.instance.CurrentSave.NewCardopediaIds.Count > 0);
		if (WorldManager.instance.GetCurrentBoardSafe().Id == "cities")
		{
			((Component)AbandonCityButton).gameObject.SetActive(true);
		}
		else
		{
			((Component)AbandonCityButton).gameObject.SetActive(false);
		}
	}
}
